...
- [Development guidelines](#development-guidelines)
    - [General Blackcoin Core](#general-blackcoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Blackcoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play blackcoins" on the test network, if you
...
([`valgrind.supp`](https://github.com/blackcoin-project/blackcoin/blob/master/contrib/valgrind.supp))
...
Some of these are maintained by active developers of Blackcoin Core, in which case changes should probably go
directly upstream without being PRed directly against the project.  They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project.  Changes to these should also
be sent upstream but bugfixes may also be prudent to PR against Blackcoin Core so that they can be integrated
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `blackcoin-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `blackcoin-cli` and the GUI debug console use this table to determine how to
